<template>
    <div class="login-container">
        ...
        <el-dialog
                ...
                center
        >
            <span>请您确认：是否删除  《 {{ activeCategroy.name }} 》  活动分类</span>
...
<script>
    ...
    import {activityCategory, activityCategoryCreate, activityCategoryDel} from '@/api/activity'
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    sort: [
                        {required: true, trigger: 'blur', message: '活动序号为必填'}
                    ]
                },
                editId: '',
                dialogDelete: false,
        ...
        methods: {
            ...
            // 删除
            deleteCategory(e) {
              if(!grant.hasAccess('activity_category.delete')){
                  this.$message({
                    type: "warning",
                    message: "无权限"
                  });
            ...
            saveCategory(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.editId) {
                            requestData.activity_category_id = this.editId
                        }
                        activityCategoryCreate(requestData).then(e => {
                            this.initiData()
                            Message({
            ...
            // 确认删除
            affirmDelete(e) {
                activityCategoryDel({activity_category_id: this.activeCategroy.id}).then(e => {
                    this.initiData()
                    Message({
                        message: '删除成功',
                        type: 'success',
